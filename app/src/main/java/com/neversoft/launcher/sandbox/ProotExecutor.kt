...
/** Keep the head and tail of over-long output, dropping the middle. */
private fun String.smartTruncate(max: Int): String {
    if (length <= max) return this
    val head = (max * 2) / 3
    val tail = max - head
    return substring(0, head) + "\n… [${length - max} chars truncated] …\n" + substring(length - tail)
}

/** Live handle to a streaming proot process (cancel / feed stdin / await exit). */
...
/**
 * Runs commands inside the Alpine rootfs via proot (`-0` fake-root, /dev /proc
 * /sys bound). Ported from the MorsVitaEst sandbox — engine only, no MVE deps.
 */
class ProotExecutor(
