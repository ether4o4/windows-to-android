...
/**
 * Downloads + extracts the Alpine minirootfs used by the proot sandbox. Ported
 * from the MorsVitaEst sandbox; the ktor HTTP client was replaced with plain
 * java.net.HttpURLConnection so NeverSoft 11 pulls in no extra dependencies.
 */
class RootfsDownloader {
    ...
    /** Try each mirror until one succeeds. Blocking — call off the main thread. */
    fun download(arch: String, targetFile: File, onProgress: (Float) -> Unit) {
    ...
    private fun downloadFrom(url: String, targetFile: File, onProgress: (Float) -> Unit) {
